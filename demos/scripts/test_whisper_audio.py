...
if __name__ == "__main__":
    ...
    # # Use the same NAO instance from the demo to say intro
    # demo.nao.tts.request(NaoqiTextToSpeechRequest("Hello! I am a Nao robot. I am going to tell a spooky story"))
    
    # # Play background sound (non-blocking)
    # try:
    #     with wave.open(audio_file, "rb") as wf:
    #         samplerate = wf.getframerate()
    #         sound = wf.readframes(wf.getnframes())
    #         message = AudioRequest(sample_rate=samplerate, waveform=sound)
    #         demo.nao.speaker.request(message, block=False)  # Non-blocking
    #         demo.play_speech_on_nao()
    # except Exception as e:
    #     demo.logger.warning("Could not play background audio: {}".format(e))
    
    # # Now play the generated speech story
